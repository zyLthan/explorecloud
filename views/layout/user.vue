...
import { todoOrderNum } from '@/api/order'
export default {
  name: 'UserPage',
  data () {
    return {
  ...
  methods: {
    ...
    async todoOrderNum () {
      const { data: { counts } } = await todoOrderNum()
      this.payment = counts.payment
      this.delivery = counts.delivery
      this.received = counts.received
      this.$store.commit('user/setOrderCounts', counts)
    }
  }

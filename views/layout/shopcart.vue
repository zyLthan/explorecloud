    ...
    <div v-if='this.$store.getters.token && cartList.length>0'>
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="isEdit===false" class="goPay" :class='{disabled:selectCount===0}' @click='goPay'>结算({{selectCount}})</div>
          <div v-else class="delete" @click="handleDelect" :class='{disabled:selectCount===0}'>删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selectCount', 'selectPrice', 'isAllChecked', 'selectCartList'])
  ...
  methods: {
    ...
    handleDelect () {
      ...
      this.$store.dispatch('cart/delectSelectAction')
      this.isEdit = false
    },
    goPay () {
      this.$router.push({
